...
:::: {.columns}
::: {.column width="60%"}
Many real-world phenomena (land use, population density, transport) are spatially heterogeneous. Classic grid-based ABMs can't capture real geography or handle map projections.
...
<svg viewBox="0 0 360 190" style="max-height:28vh; width:auto; display:block; margin:0.5em auto;">
  ...
  <rect x="236" y="72" width="83" height="78" fill="#ffffff" stroke="rgba(11,123,131,0.35)" stroke-width="1.5"/>
  <g stroke="#eee" stroke-width="1">
    <line x1="252" y1="78" x2="252" y2="156"/><line x1="268" y1="78" x2="268" y2="156"/>
    <line x1="284" y1="78" x2="284" y2="156"/><line x1="300" y1="78" x2="300" y2="156"/>
    <line x1="236" y1="94" x2="319" y2="94"/><line x1="236" y1="110" x2="319" y2="110"/>
    <line x1="236" y1="126" x2="319" y2="126"/><line x1="236" y1="142" x2="319" y2="142"/>
  </g>

  <circle cx="293" cy="118" r="4" fill="#0b7b83"/>
...
<span style="font-size:0.6em; opacity:0.75; display:block; text-align:center;">
...
* Designed for reading and writing raster datasets in Python; built on GDAL.
* Supports random access to rasters; you can read windows (subsets) into NumPy arrays for analysis or simulation.
* Provides metadata (bounds, resolution, CRS, band count) and handles coordinate transformations.
* Allows writing new rasters and updating metadata, enabling your models to generate spatial outputs.
* Integrates with libraries like `rasterstats` and `xarray` for further analysis.
...
Mesa-Geo comes with several illustrative example models.  Each demonstrates a
different combination of raster and vector data and uses Mesa-Geo’s spatial
operations in a unique way:

1. **Rainfall model** – Simulates raindrops falling on a digital elevation
   model.  Raindrops move from higher to lower elevation using the Moore
   neighbourhood and illustrate raster neighbourhood queries.
2. **Urban growth model** – Uses multiple raster layers (slope, road
   distance, land use) to determine whether a cell becomes urban.  Demonstrates
   raster overlay and weighted decisions.
3. **Population model** – Combines a population density raster with a
   vector lake boundary; agents are created based on cell populations and
   move randomly, avoiding the lake.
4. **Agents & networks model** – Uses vector layers for buildings, lakes and
   roads; commuters travel between home and workplace using shortest paths on
   the road network.
5. **Geo-Schelling (polygons)** – Agents are NUTS-2 regions (polygons).
   Each region checks its neighbours’ colours and moves if the share of
   similar neighbours falls below a threshold.
6. **Geo-Schelling (points & polygons)** – Adds point agents (people)
   residing in polygons.  People monitor the colour distribution in their
   region and relocate if unsatisfied.
